...
class ScorepadController(ControllerProtocol):
    ...
    def bind_components(self) -> None:
        ...
        self.scorepad_view.double_btn.config(command=lambda: self.set_multiplier(Multiplier.DOUBLE))
        self.scorepad_view.triple_btn.config(command=lambda: self.set_multiplier(Multiplier.TRIPLE))
        self.scorepad_view.bull25.config(command=lambda: self.on_special_press(Multiplier.OUTER_BULL))
        self.scorepad_view.bull50.config(command=lambda: self.on_special_press(Multiplier.INNER_BULL))
        self.scorepad_view.miss.config(command=lambda: self.on_special_press(Multiplier.MISS))
